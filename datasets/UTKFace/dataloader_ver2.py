"""
Dataloader cho UTKFace dataset - TỐI ƯU CHO COLAB
...
# 5 bins đều trên khoảng tuổi [0..116]
# Bins: [0-23], [24-47], [48-71], [72-95], [96-116]
AGE_BINS = np.array([0, 24, 48, 72, 96, 117], dtype=np.int64)
...
try:
    ...
    print(f"✅ Đã kết nối UTKFace HDF5 dataset:")
...
def _fetch_attr(h5_file, key, indices):
    if key not in h5_file:
        raise KeyError(
            f"Thiếu dataset '{key}' trong file HDF5. Hãy chạy lại prepare_data.py để tạo multitask labels."
        )
...
def _age_to_5_bins(age_values):
    age_values = np.asarray(age_values, dtype=np.int64)
    age_values = np.clip(age_values, AGE_BINS[0], AGE_BINS[-1] - 1)
    return np.digitize(age_values, AGE_BINS[1:-1], right=False).astype(np.int64)
...
def load_attributes(indices, category='train'):
    ...
    ages = _fetch_attr(h5_file, 'age', indices)
    genders = _fetch_attr(h5_file, 'gender', indices)
    races = _fetch_attr(h5_file, 'race', indices)

    return {
        'age': ages,
        'gender': genders,
        'race': races,
        'age5': _age_to_5_bins(ages),
    }


def load_multitask(indices, category='train'):
    X, _ = load(indices, category=category)
    attrs = load_attributes(indices, category=category)
    return X, {
        'gender': attrs['gender'],
        'age': attrs['age5'],
        'race': attrs['race'],
