...
for i in $(seq 0 "$((${shards}-1))"); do
    ...
    python sisa_cifar10.py --model cifar10 --train --slices 1 --dataset datasets/CIFAR-10/datasetfile --label "${label}" --epochs 100 --batch_size 128 --learning_rate 0.1 --optimizer sgd --chkpt_interval 1 --container "cifar10" --shard "${i}"
done